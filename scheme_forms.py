...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        return signature
        # if symbol, then define as symbol
        
       
        #also ask about the read_line thing
        # END PROBLEM 4
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
 
        # using variables signature and expressions, find defined function's
        # name (symbol), formals, body
        # Create a Lambda Procedure instance using formals and body
        # bind symbol to new Lambda Procedure
        # return symbol
        symbol = signature.first
        formals = signature.rest
        formal_check = formals
        while formal_check is not nil:
            if not scheme_symbolp(formal_check.first):
                raise SchemeError
            formal_check = formal_check.rest
        body = expressions.rest
        proc = LambdaProcedure(formals, body, env)
        env.define(symbol, proc)
        return symbol
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # body of procedure can contain mutiple expressions, but must
    # include at least one. Body is a nested Scheme list of these expressions
    # 
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    # returns first false value, otherwise defaults true
    # evaluate it somewhere
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    # returs first true value, otherwise defaults false
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            # if predicate does not have corresponding result, return
            # the predicate value
            # return value of first result sub-expression 
            # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # returns child_frame of env
    # binds symbol each element of bindings to value of corresponding expression
    # validate_form: takes in expr, min, and max. 
    # validate_formals: validates that arg is a Scheme list of symbols,
    # they must be unique 
    # 1 symbol and 1 value
    # each symbol is distinct

    # each item must be a list containing a symbol and a scheme expression
    # END PROBLEM 14
