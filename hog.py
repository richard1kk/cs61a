...
def boar_brawl(player_score, opponent_score):
    ...
    if tens_digit - ones_digit == 0:
        return 1
    return 3*abs(tens_digit - ones_digit)
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            num_0 = strategy0(score0, score1)
            score0 = update(num_0, score0, score1, dice)
            who = 1 - who
            if score0 >= goal:
                return score0, score1
        if who == 1:
            num_1 = strategy1(score1, score0)
            score1 = update(num_1, score1, score0, dice)
            who = 1 - who
            if score1 >= goal:
                return score0, score1
...
def make_averaged(original_function, times_called=1000):
    ...
    def q(*args):
        ...
        while i < times_called:
            i += 1
            val += original_function(*args)
        return val/times_called
    return q
    # END PROBLEM 8


def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    """Return the number of dice (1 to 10) that gives the
    maximum average score for a turn. Assume that the dice 
    always return positive outcomes.
    ...
    avg_max = 0
    count = 0
    average_val = make_averaged(roll_dice, times_called)
    for i in range(1,11):
        score = average_val(i, dice)
        if score > avg_max:
            avg_max = score
            count = i
    return count
